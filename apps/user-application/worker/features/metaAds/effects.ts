...
export const getPages = (query: string) =>
	Effect.gen(function* () {
		...
		// todo - if no query return no input error

		// // check cache for pagename first
		// const existingPageId = cache.getPageId(cleanQuery);
		...
		// 	pageResults,
		// 	(page) => cache.savePagename(page.name, page.page_id),
		// 	{ concurrency: 20 },
		// );
